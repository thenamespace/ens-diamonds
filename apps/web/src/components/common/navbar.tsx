...
const navItems = [
  ...
  },
  {
    name: "Favourites",
    href: "/favourites",
    render: createNavbarLink("/favourites", "Favourites"),
  },
  {
    name: "Portfolio",
    href: "/portfolio",
      ...
      <Navbar.Header className="max-w-7xl">
        <Link href="/" aria-label="ENS Diamonds home">
          <Navbar.Brand className="flex flex-row items-center gap-0">
            <Image src="/icon.png" aria-hidden className="size-10" alt="" width={40} height={40} />
        ...
        <Navbar.Content className="hidden md:flex">
          {navItems.map((item) => (
            <Navbar.Item key={item.name} isCurrent={isCurrentPath(item.href)} render={item.render}>
              {item.name}
            </Navbar.Item>
          ))}
        </Navbar.Content>
