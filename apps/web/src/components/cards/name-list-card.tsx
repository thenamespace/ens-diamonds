...
export const NameListCard = ({ name, price, ethUsd, isPricePending }: NameListCardProps) => (
  <Link className="group block" href={`/name/${name.label}.eth`}>
    <Card className="grid min-h-20 grid-cols-[auto_minmax(0,1fr)] items-center gap-3 px-4 py-3 shadow-xs transition duration-200 hover:-translate-y-0.5 hover:shadow-sm sm:grid-cols-[auto_minmax(0,1fr)_auto_minmax(9rem,auto)] sm:gap-5">
      <NameAvatar
