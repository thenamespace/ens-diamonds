...
| Situation | Result |
| --------- | ------ |
| Creator sends more than `maxSpend` during creation | `createVault` reverts |
| A non-member deposits | `deposit` reverts with `NotMember` |
| Funding reaches `maxSpend` | Further deposits revert; acquisition may start |
| A member withdraws after acquisition begins | `withdraw` reverts because the state is not `Funding` |
| Creator begins with zero escrow | `beginAcquisition` reverts |
| No commitment exists | `beginAcquisition` creates one and starts a new waiting period |
| The exact commitment already exists and is unexpired | `beginAcquisition` adopts its timestamp |
| The exact commitment is at its maximum-age boundary | `beginAcquisition` reverts for that boundary block |
| The exact commitment is fully expired | `beginAcquisition` recommits and starts a new waiting period |
| Purchase is called too early | It reverts with the earliest valid timestamp |
...
| Controller returns successfully but Safe is not the registrar owner | Purchase reverts and all transaction effects roll back |
| A member has no refund or already claimed | `claim` reverts with `NothingToClaim` |
| A claim recipient rejects ETH | That claim reverts without changing accounting |
| ETH is sent directly | `receive` or `fallback` reverts |
| ETH is forcibly sent | It is unaccounted surplus and cannot be rescued |

## Security properties

The implementation is designed to preserve these properties:

- ENS Diamonds never becomes the intended name owner
- the Safe owners, threshold, singleton, fallback handler, initializer, and deterministic address are fixed before funding
- only fixed members can add liabilities to a vault
- no member can withdraw another member's balance
...
- there is no administrator, upgrade path, fee withdrawal, arbitrary call, pause, or rescue function

The protocol does not validate ENSIP-15 normalization on-chain. An incorrectly normalized label can still match commitments built from that same string but may not represent the name users intended. Client normalization and secret generation are part of the security boundary.

## Architecture decisions

### One singleton instead of one vault contract per group

One singleton amortizes deployment cost across every group. A per-vault factory would require a new contract deployment for each attempt, create more bytecode and addresses to index, and still need shared ENS and Safe integration logic. The singleton isolates vaults by `vaultId` and member balances while `totalLiabilities` makes aggregate solvency explicit.

The tradeoff is shared contract risk. A defect in the singleton can affect every vault, while separate vault contracts would reduce the scope of some storage bugs. The implementation limits this surface with fixed states, no administration, no arbitrary execution, and minimal external integrations.

### Separate funding and acquisition phases

`createVault` starts in `Funding` rather than immediately committing. Members can verify the immutable configuration, contribute over multiple transactions, and withdraw before funds are locked. `beginAcquisition` is the explicit point after which deposits, withdrawals, and cancellation stop.

Merging creation and acquisition would save one transaction only for fully coordinated groups, while forcing the ENS commitment window to run before the vault is funded and preventing a reviewable funding phase.

### Fixed owners and strict-majority Safe

The owner list is immutable because it affects membership, Safe initialization, Safe address prediction, threshold, and refund iteration. Allowing changes would require a new commitment and address or would make the stored target inconsistent.

The strict-majority threshold is deterministic and avoids a second governance parameter. Two owners produce a 2-of-2 Safe; the protocol intentionally requires at least two members.
...
### Permissionless purchase and expiry

After the creator starts acquisition, success should not depend on the creator remaining online. Anyone with the reveal can execute `purchase`, and anyone can mark an expired attempt as failed. Permissionless execution changes who pays gas but cannot change the fixed registration or receive vault funds.

### Immutable maximum spend

`maxSpend` caps both total funding and the maximum possible ENS payment. It cannot be raised after members approve the vault configuration. A quote above escrow reverts rather than collecting more ETH during purchase.

### Two commitments

`targetIntent` binds the human target to this chain, deployment, vault, creator, duration, and private `targetSalt`. `ensCommitment` binds the exact Controller registration, including Safe owner and `ensSecret`.

Using both prevents cross-vault target reuse and ensures the on-chain registration matches the official ENS commit-reveal scheme without storing or emitting the label before purchase.

### Existing commitment adoption
...
Purchase, cancellation, and expiry do not push ETH to every contributor. Each member claims independently. This keeps lifecycle transactions bounded and prevents one rejecting recipient from blocking the group.

### Proportional surplus

A normal purchase allocates unused ETH in proportion to actual contributions, not owner count. Assigning the rounding remainder to one deterministic positive contributor preserves exact accounting without storing additional dust state.

### Cached ENS timing

Commitment ages and minimum registration duration are read once in the constructor and stored as immutables. This removes repeated external calls from lifecycle transactions and makes every vault follow the timing rules approved for that deployment.

If the external Controller were replaced or its behavior changed at the same address, the protocol would continue using its cached timing values. ENS Diamonds is therefore deployed for one known Controller instance rather than designed to follow future mutable rules.
...
All dependencies are immutable and there is no owner role. This removes privileged upgrade, pause, configuration, fee, and arbitrary-withdrawal paths. It also means a bad deployment cannot be repaired and forced ETH cannot be recovered.

## Integration checklist

Before creating a vault:

- use the correct ENS Diamonds deployment and chain ID
- normalize the label according to ENSIP-15
- keep the normalized label private until purchase
- use 2 to 10 unique owners with the creator at index zero
- generate independent high-entropy `targetSalt` and `ensSecret` values
- choose a nonzero public `vaultSalt` that the creator has not used
- derive the exact vault ID and deterministic Safe
- use a registration duration at least the cached Controller minimum and within `uint32`
- build the exact registration with zero resolver, empty data, no reverse record, and zero referrer
- derive `targetIntent` and obtain `ensCommitment` from the configured Controller
- verify `msg.value` and later deposits cannot exceed `maxSpend`

After creation:

- index `VaultCreated` to obtain the authoritative `vaultId`
- monitor each member's balance and total escrow
